...
// Arrays de índices de PS que tienen cada modificador activo.
// Usados para generar columnas por-PS en la tabla.
let _abandonoPS    = [];
let _seguridadPS   = [];
let _descansoPS    = [];
let _prioridadesPS = [];
...
// Devuelve el tiempo de abandono más próximo entre los clientes en la cola del PS[psIdx].
function proximoAbandono(estado, psIdx = 0) {
  const cola = _colaDeIdx(estado, psIdx);
  const limites = cola
    .filter(c => c[`tiempoLimite_ps${psIdx}`] !== undefined)
    .map(c => c[`tiempoLimite_ps${psIdx}`]);
...
function imprimirEstadisticas(estado) {
  ...
  // Utilización por PS: usa el acumulador per-PS _tiempoOcupado de _acumularOcupacion.
  // Si el PS aún estaba ocupado al terminar, se suma el tramo no cerrado.
  const utilizacionPS = (estado.servidores ?? []).map(ps => {
    let t = ps._tiempoOcupado ?? 0;
    if (ps._ocupadoDesde !== null && ps._ocupadoDesde !== undefined) {
      t += estado.tiempoActual - ps._ocupadoDesde;
    }
    return estado.tiempoTotal > 0 ? (t / estado.tiempoTotal * 100).toFixed(1) : "0.0";
  });

  // Fallback para edge-case sin array servidores (nunca debería ocurrir)
  if (utilizacionPS.length === 0) {
    let t = s.tiempoOcupado ?? 0;
    if (s._servidorOcupadoDesde !== null && s._servidorOcupadoDesde !== undefined) {
      t += estado.tiempoActual - s._servidorOcupadoDesde;
    }
    utilizacionPS.push(estado.tiempoTotal > 0 ? (t / estado.tiempoTotal * 100).toFixed(1) : "0.0");
  }

  const totalOcupado = (estado.servidores ?? []).reduce((acc, ps) => {
    let t = ps._tiempoOcupado ?? 0;
    if (ps._ocupadoDesde !== null && ps._ocupadoDesde !== undefined) t += estado.tiempoActual - ps._ocupadoDesde;
    return acc + t;
  }, 0);
  const utilizacionMedia = estado.tiempoTotal > 0
    ? (totalOcupado / (estado.tiempoTotal * numPS) * 100).toFixed(1)
    : "0.0";
  ...
  if (numPS === 1) {
    filas.push(["Ocupación del PS", `${utilizacionPS[0]}%`]);
  } else {
    filas.push(["Ocupación promedio", `${utilizacionMedia}%`]);
    for (let i = 0; i < utilizacionPS.length; i++) {
      filas.push([`Ocupación PS${i + 1}`, `${utilizacionPS[i]}%`]);
    }
  }
...
function imprimirEncabezadoTabla() {
  ...
  const psLbl = (base, i) => _psCount > 1 ? `${base} PS${i + 1}` : base;

  for (const i of _abandonoPS) {
    tr.append(thMod(psLbl("Próx. Abandono", i), "abandono"));
  }
  for (const i of _seguridadPS) {
    tr.append(thMod(psLbl("Llega al PS", i), "seguridad"), mkTh(psLbl("Zona Seg.", i), "seguridad"));
  }
  for (const i of _descansoPS) {
    tr.append(thMod(psLbl("Sal. Servidor", i), "descanso"), mkTh(psLbl("Reg. Servidor", i), "descanso"), mkTh(psLbl("Servidor", i), "descanso"));
  }
...
function imprimirFila({ evento, hora, estado, meta }) {
  ...
  if (_hayExtras()) {
    ...
    for (const i of _abandonoPS) {
      tr.append(tdMod(formatHora(proximoAbandono(estado, i)), "abandono"));
    }
    for (const i of _seguridadPS) {
      const zsKey = i === 0 ? "zs" : `zs_ps${i}`;
      const zsEst = i === 0 ? estado.zonaSeguridad : estado.zonasSeguridad?.[i];
      tr.append(
        tdMod(formatHora(estado._eventosExtra?.[zsKey]), "seguridad"),
        mkTd(zsEst ?? "─", "seguridad"),
      );
    }
    for (const i of _descansoPS) {
      const sKey = i === 0 ? "servidor_salida" : `servidor_salida_ps${i}`;
      const lKey = i === 0 ? "servidor_llegada" : `servidor_llegada_ps${i}`;
      const ps = estado.servidores?.[i];
      tr.append(
        tdMod(formatHora(estado._eventosExtra?.[sKey]), "descanso"),
        mkTd(formatHora(estado._eventosExtra?.[lKey]), "descanso"),
        mkTd(ps?._ausente ? "AUSENTE" : "PRESENTE", "descanso"),
      );
    }
    if (_prioridadesActivo) {
      let colaA = 0, colaB = 0;
      if (estado.topologia === "paralelo" && estado.colas) {
        for (const c of estado.colas) {
          colaA += c.filter(x => x.tipo === "A").length;
          colaB += c.filter(x => x.tipo === "B").length;
        }
      } else {
        colaA = estado.cola.filter(c => c.tipo === "A").length;
        colaB = estado.cola.filter(c => c.tipo === "B").length;
      }
      tr.append(
...
function iniciarSimulacion() {
  ...
  // Construir arrays per-PS y booleanos de modificadores activos.
  const _buildPS = (mod) => (params.psParams ?? []).reduce((acc, p, i) => {
    if (p.modificadoresActivos?.[mod]) acc.push(i);
    return acc;
  }, []);
  _abandonoPS    = _buildPS("abandono");
  _seguridadPS   = _buildPS("seguridad");
  _descansoPS    = _buildPS("descanso");
  _prioridadesPS = _buildPS("prioridades");
  _desvioPS      = _buildPS("desvio");

  _abandonoActivo    = _abandonoPS.length > 0;
  _seguridadActiva   = _seguridadPS.length > 0;
  _descansoActivo    = _descansoPS.length > 0;
  _prioridadesActivo = _prioridadesPS.length > 0;
  _desvioActivo      = _desvioPS.length > 0;
...
function applyQueueTypeConstraints() {
  for (let i = 0; i < _psCount; i++) {
    ...
    // En unafilavarios la cola es compartida: solo PS0 gestiona abandonos
    if (i > 0) {
      const abandonoCheck = panel.querySelector('.modificador-check[data-mod="abandono"]');
      const shouldDisableAbandono = _queueType === "unafilavarios";
      if (abandonoCheck) {
        abandonoCheck.disabled = shouldDisableAbandono;
        if (shouldDisableAbandono) abandonoCheck.checked = false;
      }
      const s = `_${i}`;
      const abandonoRow = document.getElementById(`param-row_abandono${s}`);
      if (abandonoRow) abandonoRow.classList.toggle("field-disabled", shouldDisableAbandono);
    }
  }
}
